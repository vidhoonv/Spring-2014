...
# use this line to convert to matrix
 ...
 pffTrain = array(dim=c(nrow(ff_train),ncol(ff_train)))
 pffTest = array(dim=c(nrow(ff_test),ncol(ff_test)))

colnames(pffTrain) = colnames(ff_train)
colnames(pffTest) = colnames(ff_test)
for(i in 1:ncol(ff_train)) {
     pffTrain[,i] = (ff_train[,i]-mean(ff_train[,i]))/sd(ff_train[,i]);
 	}


for(i in 1:ncol(ff_test)) {
     pffTest[,i] = (ff_test[,i]-mean(ff_train[,i]))/sd(ff_train[,i]);
 	}
